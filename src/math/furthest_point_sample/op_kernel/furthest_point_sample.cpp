...
private:
    __aicore__ inline void ProcessBatch(int32_t batchIdx) {
        ...
        AscendC::GlobalTensor<T> wsRdGt, wsWrGt;
        ...
        for (int32_t m = 1; m < M_; ++m) {
            ...
            for (int32_t t = 0; t < numTiles; ++t) {
                ...
                // Read minDist from workspace via DataCopy
                wsRdGt.SetGlobalBuffer(batchWs + tStart, curN);
                ...
                wsWrGt.SetGlobalBuffer(batchWs + tStart, curN);
